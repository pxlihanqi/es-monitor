...
function formatCategories(categories) {
    ...
    const uniqueDays = new Set(parsed.map((item) => dayFormatter.format(item)));
    ...
    return parsed.map((item) => {
        const text = labelFormatter.format(item);
        return isCrossDay ? text.replace("/", "-").replace(" ", " ") : text;
    });
}
function buildLineOrBarOption(monitor, data) {
...
function buildTableOption(monitor, data) {
...
export function buildChartOption(monitor, data) {
    const base = monitor.chart.type === "table"
