...
async function readAlertState(monitorDir) {
...
async function writeAlertState(monitorDir, state) {
    const stateFile = path.join(monitorDir, ".alert-state.json");
...
function buildExceptionAlertMessage(monitor, totalCount) {
...
function resolveExceptionTrendInterval(monitor) {
...
function takeLastTenBuckets(dataset) {
...
async function buildExceptionTrendImage(monitor) {
    const trendMonitor = {
        ...monitor,
...
export async function runMonitor(monitor, context) {
    ...
    if (context.dryRun) {
        logInfo(`monitor=${monitor.name} dry-run complete, image=${historyFile}, latest=${latestFile}`);
        return;
    }
    if (monitor.kind === "exception" && monitor.alert.enabled) {
        ...
        if (totalCount > threshold) {
            ...
            const cooldownActive = cooldownMinutes > 0 &&
            ...
            if (state.consecutiveHits < consecutiveCount) {
                ...
                logInfo(`monitor=${monitor.name} alert pending, total=${totalCount}, threshold=${threshold}, consecutive=${state.consecutiveHits}/${consecutiveCount}`);
                return;
            }
            if (cooldownActive) {
                await writeAlertState(monitor.monitorDir, state);
                logInfo(`monitor=${monitor.name} alert cooled down, total=${totalCount}, threshold=${threshold}, cooldownMinutes=${cooldownMinutes}`);
                return;
            }
            ...
            await writeAlertState(monitor.monitorDir, state);
            logInfo(`monitor=${monitor.name} alert sent, total=${totalCount}, threshold=${threshold}, image=${trendLatestFile}`);
        }
        else {
            state.consecutiveHits = 0;
            await writeAlertState(monitor.monitorDir, state);
