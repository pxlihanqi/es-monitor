import ms from "ms";
import type { LoadedMonitor, MonitorConfig } from "../types.js";
...
function toBeijingOffsetISOString(date: Date): string {
...
function parseAbsoluteTimeAsBeijing(value: string): string {
...
function normalizeRelativeDuration(last: string): string {
...
function resolveTimeRange(config: MonitorConfig): TimeRange {
  const now = new Date();
...
function buildMetricAgg(metric: LoadedMonitor["metrics"][number]): Record<string, unknown> {
...
function resolveBaseQuery(queryBody?: Record<string, unknown>): Record<string, unknown> {
  if (!queryBody) {
    return { match_all: {} };
  }

  if (typeof queryBody.query === "object" && queryBody.query !== null) {
    const query = queryBody.query as Record<string, unknown>;
...
function resolveFuzzyConditions(monitor: LoadedMonitor): Array<{ field: string; value: string }> {
  const fromConfig = monitor.es.fuzzyConditions ?? [];
  const fromQueryBody = Array.isArray(monitor.queryBody?.fuzzyConditions)
    ? (monitor.queryBody?.fuzzyConditions as Array<Record<string, unknown>>)
        .map((item) => ({
...
function resolveTermsFields(monitor: LoadedMonitor): string[] {
...
function resolveQuery(monitor: LoadedMonitor): Record<string, unknown> {
  // 优先使用 KQL
...
export function buildSearchBody(monitor: LoadedMonitor): Record<string, unknown> {
  const timeRange = resolveTimeRange(monitor);
  const query = resolveQuery(monitor);

  const boolFilter: Array<Record<string, unknown>> = [
    {
      range: {
        [monitor.time.field]: {
          gte: timeRange.gte,
  ...
  const termsConfig: Record<string, unknown> = {
